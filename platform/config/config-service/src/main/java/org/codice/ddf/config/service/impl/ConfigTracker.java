...
class ConfigTracker {
  ...
  private Map<String, Config> previous = Maps.newHashMap();

  private Map<String, Config> current = Maps.newHashMap();

  private Map<String, Set<Config>> fileToConfig = Maps.newHashMap();
  ...
  public ConfigEvent remove(String filename) {
    ...
    Set<String> groupIds =
        fileToConfig
            ...
            .filter(v -> v instanceof ConfigGroup)
            .map(v -> (ConfigGroup) v)
            .map(v -> v.getId())
    ...
    Set<String> singletonIds =
        fileToConfig
            ...
            .stream()
            .flatMap(Collection::stream)
            .filter(v -> v instanceof ConfigSingleton)
            .map(v -> (ConfigSingleton) v)
    ...
    ConfigEvent configEvent = new ConfigEventImpl(Stream.empty(), Stream.empty(), configs.stream());
  ...
  public ConfigEvent diff() {
    ...
    Stream<Config> updatedConfigs = entriesDiffering.values().stream().map(e -> e.rightValue());

    Map<String, Config> entriesOnlyOnRight = diff.entriesOnlyOnRight();
    Stream<Config> addedConfigs = entriesOnlyOnRight.values().stream();

    Map<String, Config> entriesOnlyOnLeft = diff.entriesOnlyOnLeft();
    Stream<Config> removedConfigs = entriesOnlyOnLeft.values().stream();
