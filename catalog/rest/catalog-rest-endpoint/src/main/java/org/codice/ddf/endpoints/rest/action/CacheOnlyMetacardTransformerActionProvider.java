...
public class CacheOnlyMetacardTransformerActionProvider extends AbstractMetacardActionProvider {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(CacheOnlyMetacardTransformerActionProvider.class);
     ...
     */
    public CacheOnlyMetacardTransformerActionProvider(String actionProviderId,
            String metacardTransformerId, ResourceCacheInterface resourceCache) {
        super();
    ...
    protected Action getAction(String metacardId, String metacardSource) {
        ...
        try {
            ...
            URI uri = new URI(SystemBaseUrl.constructUrl(PATH + "/" + metacardSource + "/"
                    + metacardId + "?transform=" + metacardTransformerId + "&" + CACHE_ONLY, true));
            url = uri.toURL();
        ...
        return new ActionImpl(getId(), TITLE_PREFIX + metacardTransformerId + " (Cache Only)",
    ...
    public <T> boolean canHandle(T subject) {
        return subject instanceof Metacard && !isCached((Metacard) subject);
    }

    private boolean isCached(Metacard metacard) {
        return resourceCache.containsValid(metacard.getSourceId() + "-" + metacard.getId(),
                metacard);
    }
}
